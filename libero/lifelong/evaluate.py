...
def main():
    ...
    with Timer() as t, VideoWriter(video_folder, args.save_videos) as video_writer:
        ...
        # 串行跑多轮，每轮env_num个episode
        num_rounds = (num_episodes + env_num - 1) // env_num
        ...
        with torch.no_grad():
            for round_idx in range(num_rounds):
                ...
                total_episodes += env_num

                print(f"[info] round {round_idx + 1}/{num_rounds}: {sum(dones)}/{env_num} success, cumulative: {num_success}/{total_episodes}")
